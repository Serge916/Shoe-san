...
class OrchestratorNode(DTROS):
    ...
    def shoepos_cb(self, new_shoe_poses):
        ...
        # Check if the trigger needs only one shoe
            ...
            self.log(f"Although new shoes arrived, the trigger behaviour is now stop or search.")
    ...
    def end_cb(self, end):
        ...
        if end.data == self.send_behavior:
            ...
            self.log(f"Unexpected end message! It was for {MISSION_DICT[end.data]}")
    ...
    def decide_mission(self):
        ...
        # If the path planner finished a new behaviour is sent
        if self.idle == True:
        ...
        if self.trigger_behavior == self.send_behavior:
            return
