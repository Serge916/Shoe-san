...
class EncoderPoseNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        rospy.Subscriber(
            f"/{self.veh}/activity_name", String, self.cbActivity, queue_size=1
        )

        rospy.Subscriber(
            f"/{self.veh}/PID_parameters", String, self.cbPIDparam, queue_size=1
        )
    ...
    def posePublisher(self):
        ...
        self.duckiebot_is_moving = (
            abs(self.delta_phi_left) > 0 or abs(self.delta_phi_right) > 0
        )
    ...
    def Controller(self):
        ...
        if self.duckiebot_is_moving:
            ...
            if self.PID_ACTIVITY:
                ...
                v, omega, self.prev_e, self.prev_int = (
                    pid_controller_homework.PIDController(
                        self.v_0,
                        self.y_ref,
                        self.y_curr,
                        self.prev_e,
    ...
    def read_params_from_calibration_file(self):
        ...
        cali_file_folder = "/code/catkin_ws/src/object-detection/assets/agent/data/config/calibrations/kinematics/"
        # cali_file_folder = "assets/data/config/calibrations/kinematics/"
        fname = cali_file_folder + self.veh + ".yaml"
        print(os.path.isdir(cali_file_folder))
